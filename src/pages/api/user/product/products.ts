...
export default async function Kwik(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "GET") {
    try {
      ...
      const User = await db.collection("products").find(
        {owner: query.toString()}).toArray() as products[]
   ...
   //PATCH request: http://localhost:3000/api/user/product/products/?product_id
   if (req.method === "PATCH") {
    try {
  ...
  //DELETE request: http://localhost:3000/api/user/product/products/?product_id
...
/**
     const getUser = async (user: string | string[]) => {
